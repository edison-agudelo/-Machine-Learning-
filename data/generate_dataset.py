...
nivel_satisfaccion = np.clip(np.round(np.random.normal(loc=0.6, scale=0.2, size=n),3), 0, 1)  # 0-1
salario = np.round(np.random.normal(loc=3000, scale=800, size=n), 2)  # unidad local
...
# Añadir ruido y convertir a etiquetas Alta/Baja (desbalance leve)
...
# Insertar valores faltantes aleatoriamente (5% en algunas columnas)
